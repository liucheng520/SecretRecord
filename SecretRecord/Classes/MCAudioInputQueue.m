...
- (BOOL)pause
{
    ...
    if (status == noErr) {
        _status = RecordStatusHold;
    }else{
        _started = RecordStatusFail;
    }
    return status == noErr;
}
...
{
    _started = NO;
    OSStatus status = AudioQueueStop(_audioQueue, false);
    if (status == noErr) {
        _status = RecordStatusEnd;
    }else{
        _started = RecordStatusFail;
    }
    return status == noErr;
